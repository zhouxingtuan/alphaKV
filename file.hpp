...
class File
{
...
#endif
	public:
	File(const std::string &name, const std::string &ext)
			: m_fileName(name + ext), m_fileLength(0),
#ifdef USE_STREAM_FILE
			  m_pFile(NULL)
#else
			  m_fileHandle(0)
	...
	inline bool saveData(const void *ptr, int64 length, int64 offset, int64 expandSize,
						 bool recordLength)
	{
		int saveLength;
		if (recordLength) {
