...
template <typename _TYPE_, uint64 _KEY_SLOT_NUMBER_> class Key : public File
{
	public:
	typedef struct KeyStorage {
		_TYPE_ value;			   // value的数值
		char key[MAX_KEY_LENGTH];  // key的值
		inline uint8 getKeyLength(void) const
		{
			return (uint8)(key[0]);
		}
		inline void setKeyLength(uint8 length)
		{
			key[0] = (char)length;
		}
		inline uint8 getEmptyLength(void) const
		{
			return (uint8)(key[1]);
		}
		inline void setEmptyLength(uint8 length)
		{
			key[1] = (char)length;
		}
	...
	typedef struct KeyValue {
		...
		inline KeyValue &operator=(const KeyValue &other)
		{
	...
	public:
	Key(const std::string &name, const std::string &ext)
			: File(name, ext), m_valueSize(0), m_keyLength(0), m_unitSize(0), m_blockSize(0)
	{
	...
	int initializeFromFile(void)
	{
		...
		if (m_valueSize != sizeof(_TYPE_)) {
			fprintf(stderr, "Key::initializeFromFile m_valueSize=%llud \n", m_valueSize);
			return FERR_KEY_VALUE_SIZE_NOT_MATCH;
		}
		if (m_keyLength > MAX_KEY_LENGTH) {
			fprintf(stderr, "Key::initializeFromFile m_keyLength=%llud \n", m_keyLength);
			return FERR_KEY_LENGTH_NOT_MATCH;
		}
		if (m_unitSize != sizeof(KeyStorage)) {
			fprintf(stderr, "Key::initializeFromFile m_unitSize=%llud \n", m_unitSize);
			return FERR_UNIT_SIZE_NOT_MATCH;
		}
		if (m_blockSize != BLOCK_SIZE) {
			fprintf(stderr, "Key::initializeFromFile m_blockSize=%llud \n", m_blockSize);
